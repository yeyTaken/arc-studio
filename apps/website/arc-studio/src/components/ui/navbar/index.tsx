...
useEffect(() => {
  function handleKeyDown(e: KeyboardEvent) {
    ...
    if (isShortcut) {
      e.preventDefault();
      setUserMenuOpen((prev) => !prev); // toggle abre/fecha
    }
  }
  ...
  return (
    <>
      ...
      <LoginCard
        ...
        userButtonRef={userButtonRef as React.RefObject<HTMLButtonElement>}
      />
      {/* final login card */}
              ...
              <li key={key}>
                {item.children ? (
                  <>
                    <button
                      onClick={() => toggleDropdown(key)}
                      className="cursor-pointer flex items-center justify-between w-full px-3 py-2 rounded hover:bg-[#141e2e75] transition-colors"
                    >
                      <div className="flex items-center gap-2">
                        {item.icon}
                        <span>{item.label}</span>
